...
function evaluate(symbol, metrics, state, options = {}) {
  ...
  const resistancePct  = metrics.resistancePct;
  ...
  const maxAutocorrelation = Number.isFinite(parseFloat(config.SNIPER_MAX_AUTOCORRELATION))
    ? parseFloat(config.SNIPER_MAX_AUTOCORRELATION)
    : -0.05;
  const bottomBreaker   = 1 - breakoutPct;  // e.g. 0.995
  const topBreaker      = 1 + breakoutPct;  // e.g. 1.005

  // A random walk (AC≈0) is not a confirmed mean-reverting regime.
  // Require negative lag-1 return correlation before any bounce/rejection.
  const meanReverting = Number.isFinite(metrics.autocorrelation) &&
    metrics.autocorrelation <= maxAutocorrelation;
  const hasBands = Number.isFinite(metrics.bbLower) && Number.isFinite(metrics.bbUpper);

  // ────────────────────────────────────────────────────────────
  //  CALL conditions (Sniper Bounce at support)
  // ────────────────────────────────────────────────────────────
  if (
    // supportPct is 0 at support and 100 at resistance.
