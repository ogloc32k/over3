    ...
    try {
      ...
      if (header) {
        ...
        if (lifecycle === 'recovering' || (active && (safeState.connectionState === 'recovering' || safeState.connectionState === 'disconnected'))) {
          ...
          const modeText = safeState.executionMode === 'real' ? 'REAL' : 'VIRTUAL';
          const modeIcon = safeState.virtualTrade ? '🧪' : '●';
          header.innerHTML = `${modeIcon} ${modeText} ${cooldownText ? `<span class="cooldown">${cooldownText}</span>` : ''} ${lockText ? `<span class="lock">${lockText}</span>` : ''}`;
          header.className = 'header-status on';
        } else {
          header.textContent = lifecycle === 'stopped' ? '● STOPPED' : '● IDLE';
      ...
      try { renderMobileView(state); } catch(e) { console.error('❌ Mobile render error:', e); }
