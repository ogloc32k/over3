...
/**
 * Formats a price to the correct decimal places for a given symbol,
 * ensuring trailing zeros are preserved.
 */
...
/**
 * Extracts the last digit from a price (handles both number and string)
 * Preserves trailing zeros by formatting with the symbol's decimal precision.
 */
function extractLastDigit(symbol, price) {
    if (price === undefined || price === null) return 0;
    const formatted = formatMarketPrice(symbol, price);
    const lastChar = formatted.slice(-1);
    return parseInt(lastChar, 10);
}

module.exports = { MARKETS, MARKET_DECIMALS, formatMarketPrice, extractLastDigit };
