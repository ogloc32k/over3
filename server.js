// server.js – full version, nothing omitted
...
// ---------- Trade‑in‑progress tracker (global lock) ----------
const tradeInProgressSym = {};   // we use a single key 'global'
...
const server = app.listen(PORT, () => {
  ...
  if (derivClient) {
    ...
    let lastTradeCloseTime = 0;           // global cooldown timestamp
    let lastProposalTime = 0;
    const lockTimestamps = {};            // timestamp when global lock was set

    // Automatic lock cleanup – force‑release stuck trades after 2 minutes
    setInterval(() => {
      const now = Date.now();
      if (tradeInProgressSym['global'] && lockTimestamps['global'] && (now - lockTimestamps['global'] > 120000)) {
        console.log('🔓 Force‑unlocking global trade – assumed settled');
        tradeInProgressSym['global'] = false;
    ...
    derivClient.on('tick', (tick) => {
      ...
      if (computed) {
        ...
        // Bot evaluation – only when active and no trade in progress
        if (store.state.active && !isTradeActive()) {
          const now = Date.now();
          // Proposal cooldown: prevent flooding
